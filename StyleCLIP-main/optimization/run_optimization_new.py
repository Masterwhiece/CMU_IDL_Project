...
from criteria.clip_image_loss import CLIPImageLoss
...
def get_lr(t, initial_lr, rampdown=0.8, rampup=0.05):
    ...
    return initial_lr * lr_ramp + 0.05
...
def main(args):
    ...
    print("Latent vector shape:", latent_code_init.shape)
    ...
    clip_image_loss = CLIPImageLoss(args)
    id_loss = IDLoss(args)
    ...
    for i in pbar:
        ...
        c_loss2 = clip_image_loss(img_gen, img_orig)
        
        text_inputs3 = torch.cat([clip.tokenize("horse")]).cuda()
        c_loss3 = clip_loss(img_gen, text_inputs3)
        ...
        if args.mode == "edit":
            ...
            # loss = c_loss + 0.00025 * l2_loss
            loss = c_loss + 0.0002 * l2_loss + 0.5 * c_loss2
        else:
            loss = c_loss
...
if __name__ == "__main__":
    ...
    parser.add_argument("--results_dir", type=str, default="results/new")
    ...
    torchvision.utils.save_image(result_image.detach().cpu(), os.path.join(args.results_dir, "final_result.jpg"), normalize=True, scale_each=True, range=(-1, 1), nrow = 5)